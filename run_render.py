...
bpy.data.scenes['Scene'].render.filepath = os.path.join(os.getcwd(),"images6",syn_image_file)
...
def q_Intersections(ob, point_in_object_space,direction):  
    #direction = Vector((drn[0],drn[1],drn[2]))  
    direction = direction.normalized()
    epsilon = direction * 1e-2  
    #count = 0  
    result, point_in_object_space, normal, index = ob.ray_cast(point_in_object_space + epsilon, direction)  
    #print(result,point_in_object_space)
    #while result:  
        #count += 1  
        #result, point_in_object_space, normal, index = ob.ray_cast(point_in_object_space + epsilon, direction)  
    #return count
...
print(os.path.join( os.getcwd(),'a4.csv'))

sc = bpy.context.scene   
bvhtree = bpy.data.objects['2']#BVHTree.FromObject(bpy.data.objects['q'], sc)  

mw = bvhtree.matrix_world
mwi = mw.inverted()

lines_count = len( open(os.path.join( os.getcwd(),'keypoints.csv')).readlines())

with open( os.path.join( os.getcwd(),'keypoints.csv')) as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    index = 0
    for row in csv_reader:
        index = index + 1
        key_3d1 = np.array([ [ float(row[0]) , float(row[1]) , float(row[2]) ]]) # backdoorbottomleft
        params = [azimuth_deg,elevation_deg,elevation_deg,rho]
        list1 = key_3d1.tolist()[0]
        origin = Vector((list1[0],list1[1],list1[2]))
        dest = mwi * camObj.matrix_world.translation
        dirn = (dest - origin).normalized()
        visible = q_Intersections(bvhtree, origin,dirn)  
        key_2d = get_3d_to_2d(key_3d1, params).astype(int)
        #print(key_2d.tolist())
        with open( os.path.join( os.getcwd(),'3.csv'),'a') as fd:
            writer = csv.writer(fd)
            csvLine = []
            csvLine.append(''.join(str( index ) ))
            csvLine.append(''.join(str(key_3d1[0][0]) ) )
            csvLine.append(''.join(str(key_3d1[0][1]) ) )
            csvLine.append(''.join(str(key_3d1[0][2]) ) )
            csvLine.append(''.join(str(key_2d[0][0]) ) )
            csvLine.append(''.join(str(key_2d[1][0]) ) )
            csvLine.append(''.join(str(visible ) ))
            csvLine.append(''.join('' ))
