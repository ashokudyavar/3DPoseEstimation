...
with open('iter1.pickle', 'rb') as file:
    [templateKeyPoints3d ,templateKeyPoints2d, templateMeanShiftedPoints,templateBins,templateNormDists,rotation_vector,translation_vector] = pickle.load(file)
    file.close()
...
INTRINSIC_CAMERA_MATRIX = np.array([ [328.1250,    0,  150],
                                      [ 0, 326.67,  84],
                                      [0,    0,    1] ],dtype="double")
success, rotation_vector, translation_vector,inliers = cv2.solvePnPRansac(points_3D, points_2D, INTRINSIC_CAMERA_MATRIX, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE, useExtrinsicGuess=True, rvec=rotation_vector, tvec=translation_vector)
